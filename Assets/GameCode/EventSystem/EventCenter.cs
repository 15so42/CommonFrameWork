...
public class EventCenter
{
    ...
    {
        // 创建一个临时列表，用于存储要删除的事件的键
        List<EnumEventType> keysToRemove = new List<EnumEventType>();

        // 遍历事件表，并将要删除的事件的键添加到临时列表中
        foreach (var key in m_EventTable.Keys)
        {
    ...
    private static void OnListenerRemoving(EnumEventType eventType, Delegate callBack)
    {
        ...
        {
           // throw new Exception(string.Format("无需移除监听：没有事件码{0}", eventType));
            Debug.LogWarning(string.Format("无需移除监听：没有事件码{0}", eventType));
    ...
    {
        if(m_EventTable.ContainsKey(eventType)==false)
            return;
